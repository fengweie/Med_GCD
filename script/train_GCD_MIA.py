...
#             print(fea.cpu().numpy().shape,logits.argmax(1).cpu().numpy().shape)
#     print(preds.shape,features.shape,preds)
...
#     student_ce = nn.Sequential(ce_backbone, ce_head).to(device)
    ####并行计算 ####
#     student_ce = torch.nn.DataParallel(student_ce)
    ####并行计算 ####
    optimizer_ce = optim.SGD(student_ce.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
...
#             print(images_.shape, class_labels.shape, uq_idxs.shape, mask_lab.shape)
#             print(torch.unique(class_labels, return_counts=True), uq_idxs.shape, torch.unique(mask_lab, return_counts=True))
#             print(torch.unique(class_labels),torch.unique(mask_lab))
            mask_lab = mask_lab[:, 0]
...
#             prototype_all = get_prototype(student_proj, student_out)
            #regularizer
...
#             print(mask_p)args.temperature
            logits_p = torch.div(
                torch.matmul(prototype_all, prototype_all.T),
                1)
#            
            mean_prob_neg = torch.log((mask_p * torch.exp(logits_p)).sum(1) / mask_p.sum(1))
            mean_prob_neg = mean_prob_neg[~torch.isnan(mean_prob_neg)]
            loss_u = mean_prob_neg.mean()
#             print(mean_prob_neg.mean(), loss_u)
#             Rw=regularizer(prototype_all)
#             loss_u=Rw.cuda()    
            sim_mat = pairwise_cosine_sim(student_feat, proto) / args.temperature
            s_dist = F.softmax(sim_mat, dim=1)
            cost_mat = pairwise_cosine_sim(student_feat, proto)
#             cost_mat = EuclideanDistances(student_fea, proto)
            ...
            # clustering, sup
            sup_logits = torch.cat([f[mask_lab] for f in (student_out / args.temperature).chunk(2)], dim=0)
            ...
            similarity_matrix = torch.matmul(student_proj_norm, student_proj_norm.T)
            
            mask_i = ~torch.eye(student_proj_norm.size(0), dtype=torch.bool).to(device)
#             similarity_matrix = similarity_matrix * mask_i
            b_ = 0.5 * int(student_proj_norm.size(0))
            logits_max, _ = torch.max(similarity_matrix, dim=1, keepdim=True)
            logits = similarity_matrix - logits_max.detach()
            
#             labels_i = torch.cat([torch.arange(b_) for i in range(2)], dim=0)
#             labels_i = (labels_i.unsqueeze(0) == labels_i.unsqueeze(1)).float()
#             labels_i = labels_i.to(device)
            # discard the main diagonal from both: labels and similarities matrix
            nominator = torch.exp(logits) 
#             print(logits,logits.shape,nominator.shape)
            denominator = torch.sum(torch.exp(logits),dim=1)
            p_weight = nominator/denominator
#             print(p_weight)
#             teacher_out = 0.3 * torch.matmul(p_weight.detach(),teacher_out) + 0.7 * teacher_out
            teacher_out = 0.3 * torch.matmul(p_weight.detach(),student_out) + 0.7 * student_out
    
            cluster_loss = cluster_criterion(student_out, teacher_out.detach(), epoch)
            avg_probs = (student_out / args.student_out_temp).softmax(dim=1).mean(dim=0)
...
class Resnet_model(nn.Module):
    def __init__(self,args):
        ...
        if args.backbone == "vit":
            self.backbone = torch.hub.load('/mnt/workdir/fengwei/NCD/MGCD/torch/facebookresearch-dino-7c446df', 'dino_vitb16',trust_repo=True, source='local')            
            # ----------------------
            ...
            self.ce_head = CE_Head(in_dim=768, out_dim=args.num_labeled_classes)
        else:   
            ...
            self.ce_head = DINOHead(in_dim=in_features, out_dim=args.mlp_out_dim, nlayers=args.num_mlp_layers)
        # ----------------------
        # PROJECTION HEAD
        # ----------------------
#         print("backbone.feat_dim",self.backbone.feat_dim)
...
#      'pathmnist'"organcmnist""organamnist""organsmnist"'tissuemnist'"bloodmnist""dermamnist"
    ...
    parser.add_argument('--student_out_temp', default=0.1, type=float, help='Final value (after linear warmup)of the student out temperature.')
...
#     if args.dataset_name == "path":
    ...
    train_dataset, test_dataset, args, datasets = get_datasets(args.dataset_name,train_transform,test_transform,args)
