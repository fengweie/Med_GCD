...
def set_args_mmf(args, train_loader):
    train_loader_target = train_loader.dataset.labelled_dataset.target + train_loader.dataset.unlabelled_dataset.datasets[0].target + train_loader.dataset.unlabelled_dataset.datasets[1].target
    ...
    print(f'Unlabeled Set Length: {len(train_loader.dataset.unlabelled_dataset.datasets[0].target + train_loader.dataset.unlabelled_dataset.datasets[1].target)}')

    train_loader_target = torch.tensor(train_loader_target)
    known_train_loader_target = train_loader_target[train_loader_target < args.num_labeled_classes]
    unknown_train_loader_target = train_loader_target[train_loader_target >= args.num_labeled_classes]
    print("total_train_loader len:{}".format(len(train_loader_target)),
          "known_train_loader len:{}".format(len(known_train_loader_target)),
          "unknown_train_loader len:{}".format(len(unknown_train_loader_target)))
#     k_cls_idx, ins_num = torch.unique(known_train_loader_target, return_counts=True)
#     print(k_cls_idx, ins_num)
...
#     print("many_thre,few_thre",int(1 / 3 * len(k_cls_idx)),int(2 / 3 * len(k_cls_idx)),many_thre,few_thre)
...
#     print(known_many_cls,known_few_cls,known_median_cls)
#     assert len(k_cls_idx) == (len(known_many_cls) + len(known_few_cls) + len(known_median_cls))

#     uk_cls_idx, ins_num = torch.unique(unknown_train_loader_target, return_counts=True)
#     val, idx = torch.sort(ins_num, descending=True)
#     print(uk_cls_idx, ins_num, val, idx)
#     if len(k_cls_idx) == 5:  # CIFAR-10
#         many_thre = val[1]
#         few_thre = val[2]
#     else:
...
#     print("many_thre,few_thre",int(1 / 3 * len(uk_cls_idx)),int(2 / 3 * len(uk_cls_idx)),many_thre,few_thre)
...
#     print(unknown_many_cls,unknown_few_cls,unknown_median_cls)
